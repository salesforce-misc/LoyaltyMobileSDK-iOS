...
final class BadgesViewModel: ObservableObject {
	...
	private func loadBadges() {
		...
		for programBadge in loyaltyProgramBadges {
			...
			for memberBadge in loyaltyProgramMemberBadges where memberBadge.loyaltyProgramBadgeId == programBadge.id {
				isMemberBadgeFound = true
				switch memberBadge.status {
				case .active:
					appendBadges(from: programBadge, and: memberBadge, type: .achieved)
				case .expired:
					appendBadges(from: programBadge, and: memberBadge, type: .expired)
				}
			}
