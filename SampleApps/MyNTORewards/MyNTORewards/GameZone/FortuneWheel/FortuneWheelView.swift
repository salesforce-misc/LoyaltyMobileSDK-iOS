				...
				ZStack {
					...
					VStack {
						...
						ZStack {
							...
							ZStack {
								if let colors: [Color] = gameDefinitionModel?.gameRewards.map({Color(hex: $0.color ?? "#FFFFFF")}),
								   let labels: [LocalizedStringKey] = gameDefinitionModel?.gameRewards.map({LocalizedStringKey($0.description ?? "description")}) {
